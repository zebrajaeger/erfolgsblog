...
public class EntryController {
    private ItemResourceRepository itemResourceRepository;
    private CacheService cacheService;

    public EntryController(ItemResourceRepository itemResourceRepository, CacheService cacheService) {
        this.itemResourceRepository = itemResourceRepository;
        this.cacheService = cacheService;
    }

    @GetMapping(path = "/api/entryResource/{id}/preview/{previewSize}")
    public byte[] getPreview(@PathVariable() long id, @PathVariable() String previewSize) throws IOException {
        ItemResource ir = itemResourceRepository.findItemResourceById(id)
                .orElseThrow(() -> new ResourceNotFoundException());

        if (ir.getType() != ItemResource.Type.IMAGE) {
